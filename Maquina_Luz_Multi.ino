...
const int ENC2 = 3;
// Pines sensores
const int muxSIG = A0; // Señal del multiplexor
const int infra1 = A3;
const int infra2 = A4;
const int infra3 = A5;
...
float shs(const char *sensor, int num_sensor) {
  ...
  // ---------- SENSORES DE LUZ ----------
    ...
    int channel = -1;
    switch (num_sensor) {
      case 1: channel = 0; break;  // canal del 4067 para sensor 1
      case 2: channel = 1; break;  // sensor 2
      case 3: channel = 2; break;  // sensor 3
      case 4: channel = 3; break;  // sensor 4
      default:
        Serial.println("Error: número de sensor light inválido");
        return -1.0;
    }

    // Selecciona canal del mux
    selectMuxChannel(channel);

    // Lee A0, que ahora está conectado al sensor elegido
...
// ----- FUNCINON MASCARA MULTIPLEXOR -----Ó
void selectMuxChannel(int ch) {
  ...
  delayMicroseconds(5); // estabilización mínima
}
...
void loop(){
  int estado = 0;
  float Si,Sd,Sm,Ci,Cd,Li,Ld,Lai,Lad; // Sensores infrarojo, de contancto y de luz
  float AVANCE = 1;
  float GIRO = 45;
  // Estado inicial
  estado = 0;
